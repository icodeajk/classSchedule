  ...
  <div style='height:100%'>
    ...
    <div class="title">
      <header-switch>
        ...
        <mt-search v-model="searchVal" slot="center" class="inner-search" cancel-text="取消" placeholder="请输入企业名、案由">
        </mt-search>
      </header-switch>
    </div>
    ...
    <scroll-view class="container" :pullup="true" @scrollToEnd="onPullingUp">
      <div>
        <div class="card-container">
          <ul class="list-ul">
            <event-card v-for="(value, index) in data" :key="index" :item="value" :eventType="'count'">
              <div slot="bottom" class="list-bottom">
                <div class="bottom-right">
                  <span>案件编号：</span>
                  <span class="high-light" v-if="value.city">{{value.city}}</span>
                  <span class="high-light" v-html="value._highlight.city[0]"></span>
                </div>
                <div class="bottom-right">
                  <span>原告：</span>
                  <span class="high-light" v-if="value.city">{{value.city}}</span>
